...
import 'trend_pie_chart.dart';

class ClassTrendsSection extends ConsumerStatefulWidget {
  const ClassTrendsSection({super.key});

  @override
  ConsumerState<ClassTrendsSection> createState() => _ClassTrendsSectionState();
}

class _ClassTrendsSectionState extends ConsumerState<ClassTrendsSection> {
        ...
        children: [
          ...
          Expanded(
            child: _selectedClass != null
                ? _buildChart()
                : const Center(child: Text('Select a class to see trends')),
          ),
        ],
      ),
    );
  ...
  Widget _buildChart() {
    ...
    return trendsAsync.when(
      ...
      data: (response) => SingleChildScrollView(
        child: TrendPieChart(points: response.trends),
      ),
    );
  }
}