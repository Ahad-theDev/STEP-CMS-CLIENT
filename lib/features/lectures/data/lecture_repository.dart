...
import 'models/my_schedule_response.dart';
...
class LectureRepository {
  ...
  Future<Lecture> updateLecture(
    ...
    final response = await dio.put(
      '${ApiConstants.lectures}/$lectureId',
      data: request.toJson(),
    );
    return Lecture.fromJson(response.data);
  }
  ...
  Future<LectureOverride> createOverride(
    String templateLectureId,
    LectureOverrideRequest request,
  ) async {
    final response = await dio.post(
  ...
  Future<MyScheduleResponse> getMySchedule({
    String? day,
    DateTime? date,
  }) async {
    final response = await dio.get(
